...
[NOTE]
====
Before 1.2.0, each released `crc` binary expired 30 days after the release due to certificates embedded in the Openshift cluster.
Using a `crc` binary older than 1.2.0 will result in an expired certificates error after 30 days.
