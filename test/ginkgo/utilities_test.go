...
// RunCRCExpectSuccessWithLogs returns stderr as well as stdout
func RunCRCExpectSuccessWithLogs(args ...string) (string, string) {
	return NewCRCCommand(args...).ExecOrDieWithLogs()
}
...
// RunOCExpectFail is a convenience wrapper over CRCBuilder
// if err != nil: return stderr, nil
// if err == nil: return stdout, err
func RunOCExpectFail(args ...string) (string, error) {
	stdout, stderr, err := Exec(exec.Command("oc", args...), nil)
	if err == nil {
		err = fmt.Errorf("Expected error but exited without error")
		return stdout, err
	}
