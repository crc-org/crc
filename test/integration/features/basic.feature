...
Feature: Basic test
    ...
    Scenario: CRC status and disk space check
        When with up to "8" retries with wait period of "2m" command "crc status --log-level debug" output should not contain "Stopped"
    ...
    Scenario: CRC status check
        When with up to "8" retries with wait period of "2m" command "crc status --log-level debug" output should not contain "Running"
