# Linux only because squid container is run by podman
# With external proxy or goproxy, this can run on all platforms
@linux @story_openshift @proxy
Feature: 4 Openshift stories behind proxy

    # Background is run after Before Scenario hooks (so proxy settings are already in place)
    Background:
    ...
    @startstop @story_health @testdata
    ...
    @mirror-registry @story_registry @testdata
    ...
    @local-registry @story_registry @testdata
