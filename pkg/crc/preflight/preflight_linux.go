...
func qemuPreflightChecks(distro *linux.OsRelease) []Check {
	checks := []Check{
		...
		{
			configKeySuffix:  "check-qemu-img-installed",
			checkDescription: "Checking if qemu-img is installed",
			check:            checkQemuImgInstalled,
			fixDescription:   "Installing qemu-img",
			fix:              fixQemuImgInstalled(distro),
