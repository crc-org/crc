...
func checkQemuImgInstalled() error {
	logging.Debug("Checking if qemu-img is installed")
	path, err := exec.LookPath("qemu-img")
	if err != nil {
		return fmt.Errorf("qemu-img was not found in path")
	}
	logging.Debugf("'qemu-img' was found at %s", path)
	return nil
}

func fixQemuImgInstalled(distro *linux.OsRelease) func() error {
	return func() error {
		logging.Debug("Trying to install qemu-img")
		stdOut, stdErr, err := crcos.RunPrivileged("Installing qemu-img", "/bin/sh", "-c", installQemuImgCommand(distro))
		if err != nil {
			return fmt.Errorf("Could not install qemu-img: %s %v: %s", stdOut, err, stdErr)
		}
		logging.Debug("qemu-img was successfully installed")
		return nil
	}
}

func installQemuImgCommand(distro *linux.OsRelease) string {
	dnfCommand := "dnf install -y qemu-img"
	switch {
	case distroIsLike(distro, linux.Ubuntu):
		return "apt-get update && apt-get install -y qemu-utils"
	case distroIsLike(distro, linux.Fedora):
		return dnfCommand
	default:
		logging.Warnf("unsupported distribution %s, trying to install qemu-img with dnf", distro)
		return dnfCommand
	}
}

func installQemuKvmCommand(distro *linux.OsRelease) string {
	dnfCommand := "dnf install -y qemu-kvm"
	switch {
	case distroIsLike(distro, linux.Ubuntu):
