...
func (client *client) Start(ctx context.Context, startConfig types.StartConfig) (*types.StartResult, error) {
	...
	if !exists {
		...
		if err := createHost(machineConfig, crcBundleMetadata.GetBundleType(), startConfig.PullSecret, startConfig.KubeAdminPassword, startConfig.DeveloperPassword); err != nil {
...
func createHost(machineConfig config.MachineConfig, preset crcPreset.Preset, pullSecret cluster.PullSecretLoader, userKubeAdminPassword string, userDeveloperPassword string) error {
	...
	if preset == crcPreset.OpenShift || preset == crcPreset.OKD {
		// Use user-provided kubeadmin password if available, otherwise generate a new one
		if userKubeAdminPassword != "" {
			logging.Infof("Using user-provided kubeadmin password from config")
			kubeAdminPassword = userKubeAdminPassword
			if err := os.WriteFile(constants.GetKubeAdminPasswordPath(), []byte(kubeAdminPassword), 0o600); err != nil {
				return errors.Wrap(err, "Error writing kubeadmin password")
			}
		} else {
		...
		// Use user-provided developer password if available, otherwise use default
		if userDeveloperPassword != "" {
			logging.Infof("Using user-provided developer password from config")
			developerPassword = userDeveloperPassword
		} else {
	...
	if err != nil {
		return fmt.Errorf("error generating cloud-init user-data: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("error in macadam during machine creation: %w\nStdout: %s\nStderr: %s", err, stdout, stderr)
	}
...
func startHost(ctx context.Context, vmName string) error {
	...
	if err != nil && !strings.Contains(stdErr, "machine did not transition into running state") {
...
func ensureRoutesControllerIsRunning(ctx context.Context, sshRunner *crcssh.Runner, ocConfig oc.Config, instanceIP string, kubeconfigFilePath string) error {
	...
	if err := ensureRestAPITokenInCluster(ctx, sshRunner, ocConfig); err != nil {
		return err
	}
...
func ensureRestAPITokenInCluster(ctx context.Context, sshRunner *crcssh.Runner, ocConfig oc.Config) error {
	token, err := restapi.LoadToken(constants.RestAPITokenPath)
	if err != nil {
		return errors.Wrap(err, "failed to load REST API token")
	}
	if err := cluster.EnsureRestAPITokenSecret(ctx, sshRunner, ocConfig, token); err != nil {
...
func copyKubeconfigFileFromVMToHost(ctx context.Context, systemdRunner *systemd.Commander, sshRunner *crcssh.Runner, kubeconfigFilePath string) error {
	...
	if err := cluster.WaitForServiceSuccessfullyFinished(ctx, systemdRunner, "ocp-cluster-ca.service", 300*time.Second, 2*time.Second); err != nil {
		return err
	}
