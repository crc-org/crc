...
func EnsureSSHKeyPresentInTheCluster(ctx context.Context, ocConfig oc.Config, sshPublicKeyPath string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to get machine configs %w: %s", err, stderr)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to update ssh key %w: %s", err, stderr)
	}
	return nil
}
...
func EnsurePullSecretPresentInTheCluster(ctx context.Context, ocConfig oc.Config, pullSec PullSecretLoader) error {
	...
	if err != nil {
		return fmt.Errorf("failed to get pull secret %w: %s", err, stderr)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to add pull secret %w: %s", err, stderr)
	}
	return nil
}
...
func EnsureGeneratedClientCAPresentInTheCluster(ctx context.Context, ocConfig oc.Config, sshRunner *ssh.Runner, selfSignedCACert *x509.Certificate, adminCert string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to get config map %w: %s", err, stderr)
	}
	...
	_, stderr, err = ocConfig.RunOcCommand(cmdArgs...)
	if err != nil {
		return fmt.Errorf("failed to patch admin-kubeconfig-client-ca config map with new CA: %w: %s", err, stderr)
	}
...
func EnsureClusterIDIsNotEmpty(ctx context.Context, ocConfig oc.Config) error {
	...
	if err != nil {
		return fmt.Errorf("failed to get clusterversion %w: %s", err, stderr)
	}
	...
	_, stderr, err = ocConfig.RunOcCommand(cmdArgs...)
	if err != nil {
		return fmt.Errorf("failed to update cluster ID %w: %s", err, stderr)
	}
...
func AddProxyConfigToCluster(ctx context.Context, sshRunner *ssh.Runner, ocConfig oc.Config, proxy *httpproxy.ProxyConfig) error {
	...
	if _, stderr, err := ocConfig.RunOcCommandPrivate(cmdArgs...); err != nil {
		return fmt.Errorf("failed to add proxy details %w: %s", err, stderr)
	}
	return nil
}
