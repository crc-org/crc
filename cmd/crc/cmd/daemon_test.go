...
func TestGatewayAPIMux_HostsEndpointsRespectModifyHostsFile(t *testing.T) {
	const token = "test-rest-api-token" // nolint:gosec
	tests := []struct {
		name             string
...
func TestGatewayAPIMux_HostsEndpointsRequireBearerToken(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "add-missing", path: "/hosts/add"},
		{name: "add-wrong", path: "/hosts/add", header: "Bearer other-token"},
		{name: "add-not-bearer", path: "/hosts/add", header: "expected-token"},
		{name: "remove-missing", path: "/hosts/remove"},
		{name: "remove-wrong", path: "/hosts/remove", header: "Bearer other-token"},
		{name: "remove-not-bearer", path: "/hosts/remove", header: "expected-token"},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			assert.False(t, hostsEditor.removeCalled)
		})
	}
}
