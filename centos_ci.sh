...
function perform_artifacts_upload() {
  ...
  if [[ "$GIT_BRANCH" = "origin/master" ]]; then
    ...
    # http://stackoverflow.com/a/22908437/1120530; Using --relative as --rsync-path not working
    mkdir -p crc/pr/$ghprbPullId/
    cp out/* crc/pr/$ghprbPullId/
    RSYNC_PASSWORD=$1 rsync -a --relative crc/pr/$ghprbPullId/ minishift@artifacts.ci.centos.org::minishift/crc/
